...
2. Gate: `dotnet build -warnaserror` clean AND `dotnet test` green AND
   `dotnet format --verify-no-changes` clean AND
...
3. If green: mark the task "- [x]" in TODO.md, run `git status --short`, stage every
   SOURCE file you created or changed this session (new files, csproj, sln),
   commit with a message naming the task. Never stage generated output
   (bin/, obj/, *.db, logs, ledger.jsonl).
...
- **This repo will be PUBLIC.** Never write a private hostname, a real LAN IP, an
  absolute home path, a key, or a reference to any other local project, company,
  vendor, or real business — not in code, not in docs, not in commit messages.
  Documentation examples use `localhost` and `192.0.2.x` addresses only. Demo data
  uses invented store and person names.
- Domain math is generic, public-domain retail-automotive arithmetic ONLY. No
  external calls of any kind anywhere in src/ — no VIN decode, no market feeds,
  nothing. The app makes zero outbound requests.
- Dependency surface is exactly: Dapper, Microsoft.Data.Sqlite, xUnit,
  Microsoft.AspNetCore.Mvc.Testing. Add anything else ONLY if your task names it.
- The desk page is one self-contained HTML file with zero external requests. No
...
- SQL migrations are append-only: never edit a committed migration; add a new one.
  The audit table is append-only in code as well — no UPDATE or DELETE touches it.
